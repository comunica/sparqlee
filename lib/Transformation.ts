...
} from './evaluators/AsyncEvaluator';
...
import {IOpenWorldTyping, isSubTypeOf} from './util/TypeHandling';
...
export function transformAlgebra(expr: Alg.Expression, creatorConfig: FunctionCreatorConfig,
  openWorldType: IOpenWorldTyping): E.Expression {
  if (!expr) {
  ...
  switch (expr.expressionType) {
    case types.TERM:
      return transformTerm(<Alg.TermExpression> expr, openWorldType);
    case types.OPERATOR:
      return transformOperator(<Alg.OperatorExpression> expr, creatorConfig, openWorldType);
    case types.NAMED:
      return transformNamed(<Alg.NamedExpression> expr, creatorConfig, openWorldType);
 ...
 */
export function transformRDFTermUnsafe(term: RDF.Term, openWorldType: IOpenWorldTyping): E.Term {
...
function transformTerm(term: Alg.TermExpression, openWorldType: IOpenWorldTyping): E.Expression {
  if (!term.term) {
  ...
  switch (term.term.termType) {
    ...
    case 'Literal': return transformLiteral(term.term, openWorldType);
 ...
 * @param openWorldType
 */
export function transformLiteral(lit: RDF.Literal, openWorldType: IOpenWorldTyping): E.Literal<any> {
  ...
  if (isSubTypeOf(dataType, TypeURL.XSD_STRING, openWorldType)) {
    return new E.StringLiteral(lit.value, dataType);
  }
  if (isSubTypeOf(dataType, DT.RDF_LANG_STRING, openWorldType)) {
    return new E.LangStringLiteral(lit.value, lit.language);
  }
  if (isSubTypeOf(dataType, DT.XSD_DATE_TIME, openWorldType)) {
  ...
  if (isSubTypeOf(dataType, DT.XSD_BOOLEAN, openWorldType)) {
  ...
  if (isSubTypeOf(dataType, DT.XSD_DECIMAL, openWorldType)) {
  ...
  if (isSubTypeOf(dataType, DT.XSD_FLOAT, openWorldType) || isSubTypeOf(dataType, DT.XSD_DOUBLE, openWorldType)) {
...
function transformOperator(expr: Alg.OperatorExpression, creatorConfig: FunctionCreatorConfig,
  openWorldType: IOpenWorldTyping): E.OperatorExpression | E.SpecialOperatorExpression {
  if (C.SpecialOperators.has(expr.operator)) {
    const specialOp = <C.SpecialOperator> expr.operator;
    const specialArgs = expr.args.map(arg => transformAlgebra(arg, creatorConfig, openWorldType));
  ...
  const regularArgs = expr.args.map(arg => transformAlgebra(arg, creatorConfig, openWorldType));
...
function wrapSyncFunction(func: SyncExtensionFunction, name: string, openWorldType: IOpenWorldTyping):
SimpleApplication {
  return args => {
    try {
      const res = func(args.map(arg => arg.toRDF()));
      return transformRDFTermUnsafe(res, openWorldType);
    } catch (error: unknown) {
...
function wrapAsyncFunction(func: AsyncExtensionFunction, name: string, openWorldType: IOpenWorldTyping):
AsyncExtensionApplication {
  return async args => {
    try {
...
function transformNamed(expr: Alg.NamedExpression, creatorConfig: FunctionCreatorConfig,
  openWorldType: IOpenWorldTyping): E.NamedExpression | E.AsyncExtensionExpression | E.SyncExtensionExpression {
  const funcName = expr.name.value;
  const args = expr.args.map(arg => transformAlgebra(arg, creatorConfig, openWorldType));
  ...
  if (creatorConfig.type === 'sync') {
    ...
    if (syncExtensionFunc) {
      const simpleAppl = wrapSyncFunction(syncExtensionFunc, expr.name.value, openWorldType);
    ...
    if (asyncExtensionFunc) {
      const asyncAppl = wrapAsyncFunction(asyncExtensionFunc, expr.name.value, openWorldType);
